"""
/admin character add - admin-only, adds a catchable character from
                  just a name + an image (uploaded as a Discord
                  attachment). It's catchable/spawnable right away,
                  ...
                  cogs/admin_edit.py.
/admin weapon add - same idea, for a Zanpakuto/weapon.
...
class AdminAdd(commands.Cog):
    ...
    @character_group.command(name="add", description="[Admin] Add a new catchable character (name + image)")
    @is_admin()
    async def character_add(
        ...
        image_path = await _save_attachment(image, name.lower().replace(" ", "_"))
    ...
    @weapon_group.command(name="add", description="[Admin] Add a new catchable weapon/zanpakuto (name + image)")
    @is_admin()
    async def weapon_add(
        ...
        await interaction.response.defer(ephemeral=True)
        image_path = await _save_attachment(image, "weapon_" + name.lower().replace(" ", "_"))

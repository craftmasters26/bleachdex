...
/admin edit character  - [bot admin] build/update the actual card for a
                        ...
                        the artwork - the "image" option is now picked
                        from card.py via autocomplete instead of an
                        upload.
...
async def _save_from_url(url: str, dest: Path) -> str:
...
async def _card_autocomplete(interaction: discord.Interaction, current: str):
    current = (current or "").lower()
    matches = [n for n in CARD_IMAGES if current in n.lower()]
...
class AdminEdit(commands.Cog):
    ...
    @app_commands.describe(
        ...
        image="Optional: pick a card artwork from card.py (spawns keep using the original image - use /admin change character for that)",
    ...
    @app_commands.autocomplete(character=_character_autocomplete, image=_card_autocomplete)
    @is_admin()
    async def card_character(
        ...
        if image is not None:
            image_url = CARD_IMAGES.get(image)
            if image_url is None:
                await interaction.followup.send(
                    f"No card mapped for **{image}** in card.py.", ephemeral=True
                )
                return
